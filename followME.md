...
# Project
 ...
 The model is applying a separable convolution on each layer which has advantges over a fully connected neural network as it preserves spatial information about the object which is required in the project like 'Follow ME'. We don't only want an object to be present on the image but also track it's position. Separable convolutions give an extra performace advantage over a regular convolutions.   Normally CNNs end up witha  fully connected layer and a softmax unit to make a decition on the which object has actually been in the input, but again this does not preserve spatial information, thus the fully connected layer has been replaiced with a 1x1 convolution with the number of kernels equal to the depth of the previous layer, this avoids flattening into a 2D tensor as it would be with a fully connected layer.  The decoder block is upsampling the image back into it's original side also applying concatenation with the smaller previous layers to preserve spatial information better. The thing is that decoder block need to _project_ the prediction made by encoder back into pixel space for a semantic segmentation purpose ( this decoder block is actually what makes it different to an object detection task ).
