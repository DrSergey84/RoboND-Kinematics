...
θ<sub>2</sub> is an angle betwwen X<sub>1</sub> and X<sub>2</sub> measured over Z<sub>2</sub>. On the picture below θ<sub>2</sub> is shown on the XY plane assosiated with frame 2, from where θ<sub>2</sub> = pi/2 - a - atan(x,y) ,where (x,y) are the coordinates of the point of WC projection on that plane. Now let's make a simple observation that frame2 origin has a fixed offset (a<sub>1</sub>, d<sub>1</sub>) from frame 0.  From the previous picture where d<sub>1</sub> was shown we know that X<sub>1</sub> is projected on hepotenus of a triangle with the sides equal (WC<sub>x</sub>, WC<sub>y</sub>) which makes it to be sqrt(WCx^2 + WCy^2), or expressed in terms of the base frame it is sqrt(WCx^2 + WCy^2) - a1, but X<sub>1</sub> is collinear with Y<sub>2</sub> as it follows from the  forward kinematics picture. Also, since the projection on XY is done along Z axis, x coordinate in terms of the base frame  would be x = WCz - d<sub>1</sub>.  
